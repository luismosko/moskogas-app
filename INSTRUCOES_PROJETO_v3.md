# INSTRUÇÕES DO PROJETO — MoskoGás v3.1
...
| Atendente | Pedidos/gestão | pedido, gestao, pagamentos, relatorio, config |
...
- Worker valida sessão em todos endpoints (exceto /api/auth/login, /health, /api/pub/*)

### Segurança
- Senhas com PBKDF2 (100k iterações)
- Endpoints `/api/auth/users` protegidos por `requireAuth(['admin'])`
...
Cria vendas no Bling em lote para pedidos pendentes. **NÃO emite NFCe** — NFCe é emitida em lote 1x/dia no painel Bling.

### NFCe
**NÃO tem endpoint direto na API Bling v3.** Emissão em lote no painel Bling. NÃO implementar via API.
...
| worker.js | v2.12.0 | Backend (wrangler deploy) |
...
### `app_users`
```sql
id INTEGER PRIMARY KEY, login TEXT UNIQUE, senha_hash TEXT,
nome TEXT, role TEXT (admin/atendente/entregador),
bling_vendedor_id INTEGER, bling_vendedor_nome TEXT,
telefone TEXT, ativo INTEGER DEFAULT 1, created_at TEXT
```

### `auth_sessions`
```sql
token TEXT PRIMARY KEY, user_id INTEGER, expires_at TEXT, created_at TEXT
```
...
### Vendedores
Endpoint `/vendedores` retorna contato.id mas SEM contato.nome. Worker faz GET `/contatos/{id}` individual para obter nome real.
...
10. Endpoints users sem auth → Usar requireAuth(['admin'])
11. Chamar "gerar-nfe" algo que cria vendas → Endpoint correto: criar-vendas-bling
...
- GET /api/auth/users | POST /api/auth/users (**admin only — requireAuth**)
...
- GET /api/bling/diagnostico | GET /api/vendedores
